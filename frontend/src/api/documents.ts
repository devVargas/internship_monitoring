...
export type DocumentDetail = DocumentReviewSummary & {
  studentEmail: string
...
type DocumentDetailResponse = DocumentReviewSummaryResponse & {
...
function isDocumentReviewSummary(value: unknown): value is DocumentReviewSummaryResponse {
  ...
  return (
    ...
    typeof value.student_registration_number === 'string' &&
...
function isDocumentActivity(value: unknown): value is DocumentActivityResponse {
  if (!isRecord(value)) {
    return false
  }
...
function isDocumentDetail(value: unknown): value is DocumentDetailResponse {
  if (!isRecord(value)) {
    return false
  }
...
function mapDocumentSummary(document: DocumentReviewSummaryResponse): DocumentReviewSummary {
  return {
    ...
    studentRegistrationNumber: document.student_registration_number,
...
function mapDocumentDetail(document: DocumentDetailResponse): DocumentDetail {
  return {
...
export async function listDocumentsForReviewRequest(
  ...
  const response = await httpClient(`/api/documents/review-queue/${suffix}`)
  const payload = await readJson(response)
  ...
  if (!Array.isArray(payload) || !payload.every(isDocumentReviewSummary)) {
    throw new Error('Resposta da fila de revisão inválida')
  }
...
export function startDocumentReviewRequest(
  ...
  return sendReviewAction(documentId, 'review', httpClient)
}
...
export function approveDocumentRequest(
  ...
  return sendReviewAction(documentId, 'approve', httpClient, comment)
}
...
export function rejectDocumentRequest(
  ...
  return sendReviewAction(documentId, 'reject', httpClient, comment)
}
